...
export function FormSelect<IForm extends FieldValues>({
    methods,
    name,
    options,
    label,
                ...
                render={({ field, fieldState: { error } }) => (
                    <>
                        ...
                        >
                            ...
                            <SelectContent>
                                {options.map((option) => (
                                    <SelectItem key={option.id} value={option[lastReturnValue]?.toString() || ''}>
                                        {option.name}
                                    </SelectItem>
                                ))}
                            </SelectContent>
                        ...
                        {!hideError && error && (
                            <ErrorMessage className="-mt-1">{error.message as string}</ErrorMessage>
                        )}
                    </>
                )}
            />
